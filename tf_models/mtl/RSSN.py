...
class RSSN(Network):
    ...
    def _inference(self):
        ...
        with tf.name_scope('utterance_encoder'):
            self.utter_encode_output, self.utter_encode_state = self._bidirectional_rnn(self.embedded_reshaped,
                                                                                        self.sent_len_reshape,
                                                                                        rnn_type='lstm')
            with tf.compat.v1.variable_scope('utter_attention'):
                _, self.alphas = attention(self.utter_encode_output, attention_size=self.attention_size,
                                           return_alphas=True)
            # reduce with attention vector
            self.utter_attened = tf.reduce_sum(self.utter_encode_output * tf.expand_dims(self.alphas, -1), 1)
            ...
            if self.dropout_keep_prob != 1:
                # [B * T, 2 * H]
                self.sent_encoder_attened_concat = tf.nn.dropout(self.sent_encoder_attened_concat,
        ...
        with tf.name_scope('dialogue_mask'):
            ...
            # [B, T] => [B, T, H]
            self.customer_mask = tf.tile(tf.expand_dims(self.role_list, axis=-1), multiples=[1, 1, self.dia_max_len])
            self.agent_mask = tf.tile(tf.expand_dims(tf.add(tf.negative(self.role_list), 1), axis=-1),
                                      multiples=[1, 1, self.dia_max_len])
            self.dia_att_mask = tf.tile(tf.expand_dims(self.dia_seq_mask, axis=-1), multiples=[1, 1, self.dia_max_len])

        with tf.name_scope('universal_context'):
            # match context
            self.cross_match = tf.matmul(self.sent_encoder_attened_concat,
                                         tf.transpose(self.sent_encoder_attened_concat, [0, 2, 1]))
            ...
            self.encode_match = tf.concat([self.sent_encoder_attened_concat, self.cross_match_sim_one_direct], axis=-1)

        with tf.name_scope('mhch_transform'):
            self.mhch_enc_dense = tf.keras.layers.Dense(units=self.dense_dim, activation='relu')(self.encode_match)
            if self.dropout_keep_prob != 1:
                self.mhch_enc_dense = tf.nn.dropout(self.mhch_enc_dense, rate=1 - self.dropout_keep_prob)

        with tf.name_scope('ssa_transform'):
            self.ssa_enc_dense = tf.keras.layers.Dense(units=self.dense_dim, activation='relu')(self.encode_match)
            if self.dropout_keep_prob != 1:
                self.ssa_enc_dense = tf.nn.dropout(self.ssa_enc_dense, rate=1 - self.dropout_keep_prob)

        with tf.name_scope('ssa2mhch_context'):
            self.ssa2mhch_matrix = tf.matmul(self.mhch_enc_dense, tf.transpose(self.ssa_enc_dense, perm=[0, 2, 1]))
            paddings = tf.ones_like(self.ssa2mhch_matrix) * (-2 ** 32 + 1)
            self.ssa2mhch_matrix_dia_masked = tf.where(tf.equal(self.dia_att_mask, 0), paddings, self.ssa2mhch_matrix)
            self.ssa2mhch_matrix_customer = tf.where(tf.equal(self.customer_mask, 0), paddings,
            ...
            self.ssa2mhch_concat = tf.concat([self.ssa2mhch_atted_vec, self.mhch_enc_dense], -1)
            self.ssa2mhch_final_vec = tf.keras.layers.Dense(units=self.dense_dim, activation='relu')(
                self.ssa2mhch_concat)

        with tf.name_scope('mhch2ssa_context'):
            self.mhch2ssa_matrix = tf.matmul(self.ssa_enc_dense, tf.transpose(self.mhch_enc_dense, perm=[0, 2, 1]))
            # position weights
            self.N, self.T = tf.shape(self.mhch2ssa_matrix)[0], tf.shape(self.mhch2ssa_matrix)[1]  # dynamic
            self.position_ind = tf.tile(tf.expand_dims(tf.range(self.T), 0), [self.N, 1])  # [N, T]
            self.dial_position_weights = tf.nn.softmax(tf.cast(self.position_ind, tf.float32))
            self.mhch2ssa_matrix = self.mhch2ssa_matrix * tf.expand_dims(self.dial_position_weights, -1)

            paddings = tf.ones_like(self.mhch2ssa_matrix) * (-2 ** 32 + 1)
            self.mhch2ssa_matrix_dia_masked = tf.where(tf.equal(self.dia_att_mask, 0), paddings, self.mhch2ssa_matrix)

            # w/o Role difference
            self.mhch2ssa_alpha = tf.nn.softmax(self.mhch2ssa_matrix_dia_masked)
            self.mhch2ssa_atted_vec = tf.matmul(self.mhch2ssa_alpha, self.mhch_enc_dense)
        ...
        with tf.name_scope('mhch_context'):
            with tf.variable_scope("mhch_context_encoding"):
                ...
                self.mhch_context_ff, _ = tf.nn.dynamic_rnn(cell=cells, inputs=self.ssa2mhch_final_vec,
                                                            sequence_length=self.dia_len, dtype=tf.float32)

        with tf.name_scope('ssa_context'):
            with tf.variable_scope('ssa_context_encoding'):
                self.ssa_context_customer = self.multihead_attention(queries=self.mhch2ssa_final_vec,
                ...
                # [B, T, Dense_dim]
                self.ssa_ff_customer = self.ff(self.ssa_context_customer, num_units=[self.ff_dim, self.dense_dim])
                self.ssa_ff_customer_distri = tf.keras.layers.Dense(units=3, activation='softmax')(self.ssa_ff_customer)
        ...
        with tf.name_scope('ssa_dis_combination'):
            ...
            # sequence mask
            self.vu_ssa_masked = tf.where(tf.equal(self.dia_seq_mask, 0), paddings, self.vu_ssa)
            # role mask
            self.vu_ssa_customer_masked = tf.where(tf.equal(self.role_list, 0), paddings, self.vu_ssa_masked)
            self.weights_local = tf.nn.softmax(self.vu_ssa_customer_masked)
            self.ssa_combine_vec = tf.reduce_sum(self.ssa_ff_customer_distri * tf.expand_dims(self.weights_local, -1),
                                                 1)

        with tf.name_scope("output"):
            # [B, T, nb_classes]
            self.main_logits = tf.keras.layers.Dense(units=2, activation='softmax')(self.mhch_context_ff)
        ...
        self.score_correct_prediction = tf.equal(tf.argmax(self.score_y, -1), self.pre_score)
        self.score_acc = tf.reduce_mean(tf.cast(self.score_correct_prediction, tf.float32))
