...
class Network(object):
    ...
    def __init__(self, vocab=None, sent_max_len=64, dia_max_len=50, nb_classes=2, nb_words=10000,
                 embedding_dim=200, dense_dim=128, rnn_dim=128, num_layers=1, lr_min=0.0, loss_lambda=1.0,
                 keep_prob=0.5, lr=0.001, weight_decay=0.0, l2_reg_lambda=0.0, warmup_steps=0,
                 optim='adam', gpu='0', memory=0, **kwargs):
        # logging
    ...
    def build_graph(self):
        ...
        self.res_dir = curdir + '/weights/' + self.data_name + '/'
        self.saver = tf.compat.v1.train.Saver()

        self.tensorboard_dir = curdir + '/tensorboard_dir/' + self.data_name + '/' + self.model_name + '/'
        ...
        self.summary_writer_train = tf.compat.v1.summary.FileWriter(self.tensorboard_dir + 'train', self.sess.graph)
        self.summary_writer_eval = tf.compat.v1.summary.FileWriter(self.tensorboard_dir + 'eval')
    ...
    def _setup_placeholders(self):
        ...
        self.role_list = tf.compat.v1.placeholder(tf.int32, [None, self.dia_max_len], name="role_list")
        self.sent_len = tf.compat.v1.placeholder(tf.int32, [None, self.dia_max_len], name='sent_len')
        self.dia_len = tf.compat.v1.placeholder(tf.int32, [None], name='dia_len')

        self.main_y = tf.compat.v1.placeholder(tf.float32, [None, self.dia_max_len, self.nb_classes], name="handoff_y")
        self.senti_y = tf.compat.v1.placeholder(tf.float32, [None, self.dia_max_len, 3], name='senti_y')
    ...
    def _inference(self):
        ...
        with tf.name_scope('sent_encoding'):
            ...
            show_layer_info_with_memory('sent_encode_output', self.sent_encoder_state, self.logger)
        if self.dropout_keep_prob != 1:
            # [B * D_len, 2 * H]
        ...
        with tf.name_scope('sequence_context_encoding'):
            # RNN
            cells = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(self.rnn_dim,
                                                                               state_is_tuple=True),
                                                  output_keep_prob=self.dropout_keep_prob)
            # [B, D_len, 2 * H]  [B, 2 * H]
            self.dia_encoder_output, self.dia_encoder_state = tf.nn.dynamic_rnn(cell=cells,
        ...
        with tf.name_scope("output"):
            # [B, D_len, nb_classes]
            self.main_logits = tf.keras.layers.Dense(units=self.nb_classes, activation='softmax')(
                self.dia_encoder_output)
            self.senti_logits = tf.keras.layers.Dense(units=3, activation='softmax')(self.dia_encoder_output)
            self.score_logits = tf.keras.layers.Dense(units=3, activation='softmax')(self.dia_encoder_state)
    ...
    def multihead_attention(self, queries, keys, values, key_masks,
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            # Attention
            outputs = self.scaled_dot_product_attention(Q_, K_, V_, key_masks, causality, dropout_rate)
    ...
    def _compute_loss(self):
        ...
        self.score_loss = tf.reduce_mean(- tf.reduce_sum(self.score_y * tf.math.log(self.score_logits + 1e-9), -1), -1)
    ...
    def _lr_with_warmup(self, warmup_step, lr_base, global_step, lr_step=20, lr_decay=0.96, staircase=False):
        ...
        if warmup_step != 0:
            ...
            exp_decay = tf.compat.v1.train.exponential_decay(lr_base, global_step - warmup_step, lr_step, lr_decay,
                                                             staircase=staircase)
            final_lr = tf.cond(global_step <= warmup_step,
    ...
    def _create_train_op(self):
        ...
        self.true_lr = self._lr_with_warmup(self.warmup_steps, self.lr, self.global_step, lr_step=50)
        ...
        if self.optim == 'adagrad':
            ...
            self.optimizer = AdamWOptimizer(weight_decay=self.weight_decay, learning_rate=self.true_lr)
        elif self.optim == 'adam':
            self.optimizer = tf.compat.v1.train.AdamOptimizer(self.true_lr)
        elif self.optim == 'sgdmoment':
            self.optimizer = tf.compat.v1.train.MomentumOptimizer(self.true_lr, 0.9, use_nesterov=True)
    ...
    def train_mhch(self, data_generator, keep_prob, epochs, data_name,
        ...
        for epoch in range(epochs):
            self.logger.info('Training the models for epoch {} with batch size {}'.format(epoch, batch_size))
            ...
            for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
                batch_handoff, batch_senti, batch_score in data_generator(mode=mode, batch_size=batch_size,
                                                                          nb_classes=nb_classes,
                                                                          shuffle=shuffle, epoch=epoch):

                feed_dict = {self.input_x1: batch_dialog_ids,
                ...
                try:
                    _, step, loss, output, scores = self.sess.run([self.train_op, self.global_step,
                    ...
                    for batch_id in range(len(batch_dia_len)):
                        ...
                        total_pre_scores.append(scores[batch_id, :batch_dia_len[batch_id], 1])
            ...
            acc, p, r, f1, macro = \
                get_a_p_r_f_sara(target=total_label_flat, predict=total_pre_label_flat)
            print(confusion_matrix(total_label_flat, total_pre_label_flat))

            gtt_1, gtt_2, gtt_3 = get_gtt_score(total_label, total_pre_label, lamb=self.lamb)

            print("MHCH Metrics %s\t: Loss:%.3f\tF1Score:%.3f\tMacro_F1Score:%.3f\tGS-I:%.3f\tGS-II:%.3f\tGS-III:%.3f"
                  % (mode, total_loss / float(counter), f1, macro, gtt_1, gtt_2, gtt_3))
            self.logger.info(
            ...
            self.logger.info("#############################################")
            if is_val:
                eval_loss, eval_acc, eval_f1, eval_macro, eval_auc, gs1, gs2, gs3 = \
                    self.evaluate_batch_mhch(data_generator, data_name, mode=val_mode,
                                             batch_size=batch_size, nb_classes=nb_classes, shuffle=False)

                metrics_dict = {"loss": eval_loss, "acc": eval_acc, "f1": eval_f1,
                                "macro": eval_macro, "gs1": gs1, "gs2": gs2, "gs3": gs3}
        ...
        if is_test:
            ...
            self.evaluate_batch_mhch(data_generator, data_name, mode=test_mode,
                                     batch_size=batch_size, nb_classes=nb_classes, shuffle=False)
        self.summary_writer_train.close()
        self.summary_writer_eval.close()

    def evaluate_batch_mhch(self, data_generator, data_name, mode='eval',
                            batch_size=32, nb_classes=2, shuffle=False):
        counter, total_loss = 0, 0.0
        total_label = []
        total_pre_label = []
        total_pre_scores = []
        ...
        for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
            batch_handoff, batch_senti, batch_score in data_generator(mode=mode,
            ...
            try:
                loss, output, probas = self.sess.run([self.loss, self.output, self.proba],
                                                     feed_dict)
                # for handoff metric
                true_y = np.argmax(batch_handoff, -1)
                for batch_id in range(len(batch_dia_len)):
                    total_label.append(true_y[batch_id, :batch_dia_len[batch_id]])
                    total_pre_label.append(output[batch_id, :batch_dia_len[batch_id]])
                    total_pre_scores.append(probas[batch_id, :batch_dia_len[batch_id], 1])
        ...
        acc, p, r, f1, macro = get_a_p_r_f_sara(target=total_label_flat, predict=total_pre_label_flat)
        print(confusion_matrix(total_label_flat, total_pre_label_flat))
        gtt_1, gtt_2, gtt_3 = get_gtt_score(total_label, total_pre_label, lamb=self.lamb)
        ...
        print(
            ...
            % (mode, total_loss / float(counter), f1, macro, auc_score, gtt_1, gtt_2, gtt_3))
        self.logger.info(
            "MHCH Eval Metrics %s: Loss:%.3f\tF1Score:%.3f\tMacro_F1Score:%.3f\tAUC:%.3f\tGS-I:%.3f\tGS-II:%.3f\tGS-III:%.3f"
            % (mode, total_loss / float(counter), f1, macro, auc_score, gtt_1, gtt_2, gtt_3))
        ...
        if mode == 'test':
            ...
            self.logger.info(
                ...
                % (mode, f1, macro, auc_score, gtt_1, gtt_2, gtt_3))

            self.logger.info("\n" + classification_report(total_label_flat, total_pre_label_flat, digits=4))
            self.logger.info("\n" + str(confusion_matrix(total_label_flat, total_pre_label_flat)))
            tmp_lambda = 0.99
            gtt_1, gtt_2, gtt_3 = get_gtt_score(total_label, total_pre_label, lamb=tmp_lambda)
        ...
        return total_loss / float(counter), acc, f1, macro, auc_score, gtt_1, gtt_2, gtt_3

    def train_ssa(self, data_generator, keep_prob, epochs, data_name,
                  mode='train', batch_size=20, nb_classes=2, shuffle=True,
                  is_val=True, is_test=True, save_best=True,
                  val_mode='eval', test_mode='test'):
        ...
        for epoch in range(epochs):
            ...
            for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
                ...
                feed_dict = {self.input_x1: batch_dialog_ids,
                             ...
                             self.dropout_keep_prob: keep_prob}
                try:
                    _, step, loss, output = self.sess.run([self.train_op, self.global_step,
            ...
            f1_0, f1_1, f1_2 = f1_score(label_list, pre_label_list, labels=[0, 1, 2], average=None)

            print(confusion_matrix(label_list, pre_label_list))
            print("SSA Training Metrics %s\t: Loss:%.3f\tWS F1:%.3f\tMT F1:%.3f\tUS F1:%.3f\tMacro F1:%.3f\tAcc.:%.3f"
                  % (mode, total_loss / float(counter), f1_2, f1_1, f1_0, macro_f1, acc))
            self.logger.info(
            ...
            if is_val:
                ...
                eval_loss, eval_f1_2, eval_f1_1, eval_f1_0, eval_macro, eval_acc = \
                    self.evaluate_batch_ssa(data_generator, data_name, mode=val_mode,
                                            batch_size=batch_size, nb_classes=nb_classes, shuffle=False)

                metrics_dict = {"loss": eval_loss, "acc": eval_acc, "macro": eval_macro,
                                "f1_0": eval_f1_0, "f1_1": eval_f1_1, "f1_2": eval_f1_2}
        ...
        if is_test:
            test_t = time.time()
            self.restore(self.save_dir, self.model_name + '.best')
            self.evaluate_batch_ssa(data_generator, data_name, mode=test_mode,
                                    batch_size=batch_size, nb_classes=nb_classes, shuffle=False)
        self.summary_writer_train.close()
        self.summary_writer_eval.close()

    def evaluate_batch_ssa(self, data_generator, data_name, mode='eval',
                           batch_size=32, nb_classes=2, shuffle=False):
        counter, total_loss = 0, 0.0
        total_label = []
        total_pre_label = []
        ...
        for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
            ...
            feed_dict = {self.input_x1: batch_dialog_ids,
                         ...
                         self.dropout_keep_prob: 1.0}
            try:
                loss, output = self.sess.run([self.loss, self.output],
                                             feed_dict)
                true_y = np.argmax(batch_score, -1)
                total_label.append(true_y)
                total_pre_label.append(output)
        ...
        if mode == 'test':
            ...
            self.logger.info(
                ...
                % (mode, f1_2, f1_1, f1_0, macro_f1, acc))
    ...
    def train_crf(self, data_generator, keep_prob, epochs, data_name,
                  mode='train', batch_size=32, nb_classes=2, shuffle=True,
                  is_val=True, is_test=True, save_best=True,
                  val_mode='eval', test_mode='test'):
        print('Using sequence labeling crf trainer.')
        max_val = 0
        for epoch in range(epochs):
            self.logger.info('Training the models for epoch {} with batch size {}'.format(epoch, batch_size))
            ...
            for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
                labels_padded, batch_senti, batch_score in data_generator(mode=mode, batch_size=batch_size,
                ...
                feed_dict = {self.input_x1: batch_dialog_ids,
                             ...
                             self.dia_len: batch_dia_len,
                             self.sent_len: batch_sent_len,
                             self.main_y: labels_padded,
                             self.dropout_keep_prob: keep_prob}
                try:
                    _, step, loss, sequence = self.sess.run([self.train_op, self.global_step,
                                                             self.loss, self.viterbi_sequence],
                                                            feed_dict)
                    for batch_id in range(len(batch_dia_len)):
                        total_label.append(labels_padded[batch_id, :batch_dia_len[batch_id]])
                        total_pre_label.append(sequence[batch_id, :batch_dia_len[batch_id]])
                    total_loss += loss
                    counter += 1
    ...
    def evaluate_batch_crf(self, data_generator, data_name, mode='eval',
        ...
        for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
            labels_padded, batch_senti, batch_score in data_generator(mode=mode,
                                                                      batch_size=batch_size, nb_classes=2,
                                                                      shuffle=shuffle):
            ...
            feed_dict = {self.input_x1: batch_dialog_ids,
                         ...
                         self.dropout_keep_prob: 1.0}
            try:
                loss, sequence, probas = self.sess.run([self.loss, self.viterbi_sequence, self.proba], feed_dict)
    ...
    def train_mt(self, data_generator, keep_prob, epochs, data_name,
                 mode='train', batch_size=20, nb_classes=2, shuffle=True,
                 is_val=True, is_test=True, save_best=True,
                 val_mode='eval', test_mode='test'):

        print('Using multi-task classification trainer.')
        max_val = 0
        ...
        for epoch in range(epochs):
            ...
            for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
                ...
                feed_dict = {self.input_x1: batch_dialog_ids,
                             ...
                             self.dropout_keep_prob: keep_prob}
                try:
                    _, step, summaries, loss, sequence, senti, score = self.sess.run(
                        [self.train_op, self.global_step, self.summaries,
                         self.loss, self.output, self.senti, self.pre_score], feed_dict)
                    ...
                    for batch_id in range(len(batch_dia_len)):
                        ...
                        total_pre_handoff.append(sequence[batch_id, :batch_dia_len[batch_id]])

                        total_senti.append(senti_y[batch_id, :batch_dia_len[batch_id]])
                        total_pre_senti.append(senti[batch_id, :batch_dia_len[batch_id]])
            ...
            # handoff
            gtt_1, gtt_2, gtt_3 = get_gtt_score(total_handoff, total_pre_handoff, lamb=self.lamb)
            acc_handoff, p_handoff, r_handoff, f1_handoff, macro_handoff = \
                get_a_p_r_f_sara(target=total_handoff_flat, predict=total_pre_handoff_flat)
            ...
            print(
                ...
                % (mode, total_loss / float(counter), f1_handoff, macro_handoff, gtt_1, gtt_2, gtt_3))
            self.logger.info(
                "MHCH MT Metrics %s: Loss:%.3f\tF1Score:%.3f\tMacro_F1Score:%.3f\tGS-I:%.3f\tGS-II:%.3f\tGS-III:%.3f"
                % (mode, total_loss / float(counter), f1_handoff, macro_handoff, gtt_1, gtt_2, gtt_3))
            ...
            ssa_f1_0, ssa_f1_1, ssa_f1_2 = f1_score(score_list, pre_score_list, labels=[0, 1, 2], average=None)
            ...
            print(
                ...
                % (mode, total_loss / float(counter), ssa_f1_2, ssa_f1_1, ssa_f1_0, ssa_macro_f1, ssa_acc))
            self.logger.info(
                "SSA MT Training Metrics %s\t: Loss:%.3f\tWS F1:%.3f\tMT F1:%.3f\tUS F1:%.3f\tMacro F1:%.3f\tAcc.:%.3f"
                % (mode, total_loss / float(counter), ssa_f1_2, ssa_f1_1, ssa_f1_0, ssa_macro_f1, ssa_acc))
            self.logger.info("#############################################")
            if is_val:
                eval_loss, eval_f1_handoff, eval_macro_handoff, eval_auc, gs1, gs2, gs3, \
                eval_f1_2_ssa, eval_f1_1_ssa, eval_f1_0_ssa, eval_macro_ssa, eval_acc_ssa = \
                    self.evaluate_batch_mt(data_generator, data_name, mode=val_mode, global_step=step,
                                           batch_size=batch_size, nb_classes=nb_classes, shuffle=False)
                total_macro = eval_macro_handoff + eval_macro_ssa
                total_metrices = eval_macro_handoff + gs3 + eval_macro_ssa
                metrics_dict = {"loss": eval_loss, "f1_handoff": eval_f1_handoff, "macro_handoff": eval_macro_handoff,
                                "gs1": gs1, "gs2": gs2, "gs3": gs3,
                                "acc_ssa": eval_acc_ssa, "macro_ssa": eval_macro_ssa,
                                "f1_0_ssa": eval_f1_0_ssa, "f1_1_ssa": eval_f1_1_ssa, "f1_2_ssa": eval_f1_2_ssa,
                                "total_macro": total_macro, "total_metrices": total_metrices}
        ...
        if is_test:
            self.restore(self.save_dir, self.model_name + '.best')
            self.evaluate_batch_mt(data_generator, data_name, mode=test_mode, global_step=0,
                                   batch_size=batch_size, nb_classes=nb_classes, shuffle=False)

        self.summary_writer_train.close()
        self.summary_writer_eval.close()

    def evaluate_batch_mt(self, data_generator, data_name, global_step, mode='eval',
                          batch_size=32, nb_classes=2, shuffle=False):
        counter, total_loss = 0, 0.0
        ...
        for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
            ...
            try:
                ...
                for batch_id in range(len(batch_dia_len)):
                    total_handoff.append(handoff_y[batch_id, :batch_dia_len[batch_id]])
                    total_pre_handoff.append(sequence[batch_id, :batch_dia_len[batch_id]])
                    total_pre_handoff_scores.append(handoff_prob[batch_id, :batch_dia_len[batch_id], 1])

                    for idx, flag in enumerate(batch_role_ids[batch_id, :batch_dia_len[batch_id]]):
                        if int(flag) == 1:
        ...
        acc_handoff, p_handoff, r_handoff, f1_handoff, macro_handoff = \
            get_a_p_r_f_sara(target=total_handoff_flat, predict=total_pre_handoff_flat)
        # calc AUC score
        fpr, tpr, thresholds = roc_curve(total_handoff_flat, total_pre_handoff_scores_flat, pos_label=1)
        auc_score = auc(fpr, tpr)

        print(confusion_matrix(total_handoff_flat, total_pre_handoff_flat))
        print("MHCH MT Eval Metrics %s: F1Score:%.3f\tMacro_F1Score:%.3f\tAUC:%.3f\tGS-I:%.3f\tGS-II:%.3f\tGS-III:%.3f"
              % (mode, f1_handoff, macro_handoff, auc_score, gtt_1, gtt_2, gtt_3))
        self.logger.info(
        ...
        senti_macro_f1 = f1_score(total_senti_flat, total_pre_senti_flat, average='macro')
        senti_f1_0, senti_f1_1, senti_f1_2 = f1_score(total_senti_flat, total_pre_senti_flat, labels=[0, 1, 2],
                                                      average=None)
        ...
        print(confusion_matrix(score_list, pre_score_list))
        print("SSA Eval Metrics %s\t: Loss:%.3f\tWS F1:%.3f\tMT F1:%.3f\tUS F1:%.3f\tMacro F1:%.3f\tAcc.:%.3f"
              % (mode, total_loss / float(counter), ssa_f1_2, ssa_f1_1, ssa_f1_0, ssa_macro_f1, ssa_acc))
        self.logger.info(
        ...
        if mode == 'test':
            ...
            self.logger.info(
                "Metrics %s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f"
                % (mode, f1_handoff, macro_handoff, auc_score, gtt_1, gtt_2, gtt_3))
            self.logger.info("\n" + classification_report(total_handoff_flat, total_pre_handoff_flat, digits=4))
            ...
            self.logger.info(
                "Metrics %s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f"
                % (mode, senti_f1_2, senti_f1_1, senti_f1_0, senti_macro_f1, senti_acc))
            self.logger.info("\n" + classification_report(total_senti_flat, total_pre_senti_flat, digits=4))
            self.logger.info("\n" + str(confusion_matrix(total_senti_flat, total_pre_senti_flat)))

            self.logger.info(
                "SSA Test Metrics:\tWS F1\tMT F1\tUS F1\tMacro F1\tAcc.")
            self.logger.info(
                "Metrics %s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f"
                % (mode, ssa_f1_2, ssa_f1_1, ssa_f1_0, ssa_macro_f1, ssa_acc))
            self.logger.info("\n" + classification_report(score_list, pre_score_list, digits=4))
            self.logger.info("\n" + str(confusion_matrix(score_list, pre_score_list)))
            tmp_lambda = 0.99
            gtt_1, gtt_2, gtt_3 = get_gtt_score(total_handoff, total_pre_handoff, lamb=tmp_lambda)
            self.logger.info("Lambda={}\t{}\t{}\t{}".format(tmp_lambda, gtt_1, gtt_2, gtt_3))
            tmp_lambda = 0.75
            gtt_1, gtt_2, gtt_3 = get_gtt_score(total_handoff, total_pre_handoff, lamb=tmp_lambda)
            self.logger.info("Lambda={}\t{}\t{}\t{}".format(tmp_lambda, gtt_1, gtt_2, gtt_3))
            ...
            val_summary = tf.compat.v1.Summary(value=[
                tf.compat.v1.Summary.Value(tag="loss", simple_value=total_loss / float(counter)),
                tf.compat.v1.Summary.Value(tag="metrics/score_acc", simple_value=ssa_acc),
                tf.compat.v1.Summary.Value(tag="metrics/GS-III", simple_value=gtt_3 * 100)
            ])
            self.summary_writer_eval.add_summary(val_summary, global_step)

        return total_loss / float(counter), f1_handoff, macro_handoff, auc_score, gtt_1, gtt_2, gtt_3, \
               ssa_f1_2, ssa_f1_1, ssa_f1_0, ssa_macro_f1, ssa_acc
